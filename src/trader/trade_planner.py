...
class TradePlan:
    ...
    risk_reward: float
    invalidation: float
    target_is_structural: bool = True
...
class TradePlanner:
    ...
    def plan(
        ...
        *,
        ...
        if direction is Direction.LONG:
            ...
            if risk <= 0.0:
                return None
            take_profit, structural = self._long_take_profit(
                entry, risk, target_rr, pivots.swing_highs
            )
            ...
            if risk <= 0.0:
                return None
            take_profit, structural = self._short_take_profit(
                entry, risk, target_rr, pivots.swing_lows
            )
        ...
        return TradePlan(
            ...
            invalidation=level,
            target_is_structural=structural,
        )
    ...
    def _long_take_profit(
        ...
        Returns the target and whether it is the pivot itself (structural) rather than the
        floor. The distinction is invisible in ``risk_reward`` — both satisfy the ratio —
        so it is returned rather than inferred.
        """
        ...
        return (nearest, True) if nearest >= floor else (floor, False)
    ...
    def _short_take_profit(
        ...
        Mirror of the long case, returning the same structural/manufactured distinction.
        """
